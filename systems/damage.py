...
def damage_messages(results, attacker, target, base_damage, final_damage, damage_type):
	if final_damage > 0:
		results.append({'message': Message('{0} hits {1} for {2} hit points.'.format(attacker.name.capitalize(), target.name, str(base_damage)), libtcod.white)})
		...
		results.append({'message': Message('{0} doesn\'t do enough damage to penetrate the {1}\'s armour.'.format(attacker.name.capitalize(), target.name), libtcod.white)})
